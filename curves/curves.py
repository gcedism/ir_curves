...
class Curves :
    ...
    def updateDeposits(self) :
        self._deposits = self._depositsHistory[self._country].loc[self._pricing_dt, [30, 90, 180]]
    ...
    def updateGovt(self) :
        self._govt = self._govtHistory[self._country].loc[self._pricing_dt]
         
    def updateZero(self) :
        
        _zero = pd.DataFrame(self._deposits).rename(columns={self._pricing_dt : 'rate'})
        _zero['df'] = 1 / ( 1 + _zero['rate'] * _zero.index / 36000 )
        
        next_ust = self._govt.loc[self._govt.index > _zero.index[-1]]
        max_j = 20
        j = 0
        while (not next_ust.empty and j <= max_j) :
            
            _maturity = self._pricing_dt + td(days=int(next_ust.index[0]))
            _coupon = next_ust.iloc[0]
            ust = self.cash_flow(_maturity, _coupon, pricing_dt=self._pricing_dt)
            ust['zero_df'] = ust.join(_zero, how='outer').astype(float).interpolate().loc[ust.index, 'df']
